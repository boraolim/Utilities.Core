...
namespace TestUtilities
{
  public class App
  {
    ...
    public App(IConfiguration config, IToolService iToolSvc, IWhereClauseGeneratorService whereClauseGeneratorSvc, IDummieData dummieData,
    ...
    public async Task RunAsync(string[] args)
    {
      try
      {
        ...
        var _newFilter = new WhereFilter()
        {
          ...
          {
            new WhereRule() { field = "Name", op = WhereConditions.eq, data = "xxx" },
            new WhereRule() { field = "Id", op = WhereConditions.eq, data = System.Guid.NewGuid().ToString() }
          }
        };
        ...
        /* XML Serializacion. */
        var _strXML = await _iXmlSerializationService.XmlSerializeAsybc(_rommieData);
        ...
        _iDbManagerSQLServer.ProveedorDatos = DataBaseProviders.SQLServer;
